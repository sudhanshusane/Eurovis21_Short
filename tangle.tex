...
\end{subfigure}
\vspace{-2mm}
\caption{Visualization of the analytical tangle function with a focus on uncertainty in linking regions between multiple blobs. We used $T=[0,62]$. As expected for this data set, we found $FCLS_{T,C}$~(visualized as 25\% opacity level-sets) are visible in the linking regions and form wider envelopes as the confidence interval increases from 50\%~(c) to 68\%~(d) to 95\%~(e).} 
